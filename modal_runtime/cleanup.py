...
def cleanup_stale_runs(
    ...
    try:
        ...
        if dry_run:
            ...
            result = supabase.table("runs").select("id, user_id, updated_at, status").in_(
                "status", ["initialized", "running"]
            ).lt("updated_at", cutoff_time.isoformat()).execute()
            ...
            for run in stale_runs:
                ...
                minutes_stale = (datetime.now(timezone.utc) - updated_at).total_seconds() / 60
            ...
            result = supabase.rpc('mark_stale_runs_as_failed', {
