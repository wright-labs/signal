...
def get_s3_bucket_name() -> str:
    ...
    if not bucket:
        raise ValueError(
            "Neither R2_BUCKET_NAME nor S3_BUCKET_NAME environment variable is set"
        )
    return bucket
...
def upload_file(
    local_path: str, s3_path: str, bucket: Optional[str] = None
) -> Dict[str, Any]:
...
def upload_directory(
    local_path: str, s3_prefix: str, bucket: Optional[str] = None
) -> Dict[str, Any]:
    ...
    logger.info(
        f"Uploaded {len(uploaded_files)} files ({total_bytes} bytes) to s3://{bucket}/{s3_prefix}"
    )
...
def upload_manifest(
    manifest_dict: Dict[str, Any], s3_prefix: str, bucket: Optional[str] = None
) -> Dict[str, Any]:
...
def generate_signed_url_for_prefix(
    s3_prefix: str, expiration: int = 3600, bucket: Optional[str] = None
) -> str:
...
def delete_run_artifacts(
    owner_id: str, run_id: str, bucket: Optional[str] = None
) -> Dict[str, Any]:
