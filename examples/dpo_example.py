...
def main():
    ...
    # Initialize client
    client = rs.Client(
        api_key="your-api-key-here", base_url="https://api.rewardsignal.com/v1"
    )
    ...
    for epoch in range(num_epochs):
        ...
        if "preference_accuracy" in result.grad_stats:
            print(
                f"  Preference Accuracy: {result.grad_stats['preference_accuracy']:.2%}"
            )
...
def advanced_dpo_example():
    ...
    for batch_start in range(0, len(full_dataset), batch_size):
        ...
        if not accumulate:
            ...
            print(
                f"Step {step // accumulation_steps}: Loss={result.loss:.4f}, Beta={current_beta:.3f}"
            )
