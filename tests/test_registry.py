...
class TestRunUpdates:
    ...
    def test_update_appends_metrics(self, run_registry, test_user_id):
        ...
        # Add first metric
        run_registry.update_run(run_id, current_step=0, metrics={"loss": 2.5, "grad_norm": 1.2})
        
        # Add second metric
        run_registry.update_run(run_id, current_step=1, metrics={"loss": 2.3, "grad_norm": 1.1})
...
class TestMetricsManagement:
    ...
    def test_metrics_preserve_order(self, run_registry, test_user_id):
        ...
        for i in range(5):
            run_registry.update_run(run_id, current_step=i, metrics={"loss": 3.0 - i * 0.1})
