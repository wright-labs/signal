...
class TestAllocateGPUConfig:
    ...
    def test_7b_to_13b_models(self):
        ...
        assert (
            allocate_gpu_config("Qwen/Qwen2.5-7B", parameter_count=7.0) == "A100-80GB:1"
        )
        assert (
            allocate_gpu_config("test/model-8B", parameter_count=8.0) == "A100-80GB:1"
        )
        assert (
            allocate_gpu_config("test/model-12B", parameter_count=12.9) == "A100-80GB:1"
        )

    def test_13b_to_30b_models(self):
        """Test 13B-30B parameter models get A100-80GB:2."""
        assert (
            allocate_gpu_config("test/model-13B", parameter_count=13.0) == "A100-80GB:2"
        )
        assert (
            allocate_gpu_config("test/model-14B", parameter_count=14.0) == "A100-80GB:2"
        )
        assert (
            allocate_gpu_config("test/model-29B", parameter_count=29.9) == "A100-80GB:2"
        )

    def test_30b_to_70b_models(self):
        """Test 30B-70B parameter models get A100-80GB:4."""
        assert (
            allocate_gpu_config("test/model-30B", parameter_count=30.0) == "A100-80GB:4"
        )
        assert (
            allocate_gpu_config("test/model-32B", parameter_count=32.0) == "A100-80GB:4"
        )
        assert (
            allocate_gpu_config("test/model-69B", parameter_count=69.9) == "A100-80GB:4"
        )

    def test_over_70b_models(self):
        """Test > 70B parameter models get A100-80GB:8."""
        assert (
            allocate_gpu_config("test/model-100B", parameter_count=100.0)
            == "A100-80GB:8"
        )
    ...
    def test_user_override(self):
        ...
        assert (
            allocate_gpu_config(
                "test/model-8B", user_override="L40S:2", parameter_count=8.0
            )
            == "L40S:2"
        )
        assert (
            allocate_gpu_config(
                "test/model-70B", user_override="H100:4", parameter_count=70.0
            )
            == "H100:4"
        )
