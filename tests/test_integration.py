...
class TestBasicTrainingWorkflow:
    ...
    def test_complete_training_flow(
        self, test_client, test_api_key, sample_batch, sample_prompts
    ):
...
class TestGradientAccumulation:
    ...
    def test_gradient_accumulation_workflow(
        self, test_client, test_api_key, sample_batch
    ):
    ...
    def test_alternating_accumulation_and_steps(
        self, test_client, test_api_key, sample_batch
    ):
...
class TestCustomConfigurations:
    ...
    def test_custom_lora_config(self, test_client, test_api_key):
        ...
        assert run.config["lora_target_modules"] == [
            "q_proj",
            "v_proj",
            "k_proj",
            "o_proj",
    ...
    def test_different_sampling_parameters(self, test_client, test_api_key):
        ...
        result2 = run.sample(
            prompts=["Test"], temperature=1.5, top_p=0.95, max_tokens=100
        )
...
class TestErrorScenarios:
    ...
    def test_invalid_model_name(self, test_client, test_api_key):
        ...
        assert "not supported" in str(exc_info.value).lower() or "400" in str(
            exc_info.value
        )

    def test_unauthorized_run_access(
