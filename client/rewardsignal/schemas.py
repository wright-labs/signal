...
class TrainingExample(BaseModel):
    ...
    text: Optional[str] = Field(
        None, max_length=32768, description="Raw text for SFT (max 32K chars)"
    )
    ...
    prompt: Optional[str] = Field(
        None, max_length=32768, description="Prompt for preference-based methods"
    )
    ...
    def validate_format(self) -> "TrainingExample":
        ...
        has_dpo = all([self.prompt is not None, self.chosen is not None, self.rejected is not None])
        has_grpo = all(
            [self.prompt is not None, self.responses is not None, self.rewards is not None]
        )
        has_ppo = all([self.prompt is not None, self.response is not None, self.reward is not None])
...
class ForwardBackwardRequest(BaseModel):
    ...
    batch_data: List[TrainingExample] = Field(
        ..., min_length=1, max_length=128, description="List of training examples (1-128)"
    )
    accumulate: bool = Field(False, description="Accumulate gradients instead of replacing")
    loss_fn: str = Field(
        "causal_lm", description="Loss function to use (causal_lm, dpo, grpo, ppo)"
    )
