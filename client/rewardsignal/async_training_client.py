"""Async training-specialized client for Signal API."""
...
class AsyncTrainingClient:
    """Async specialized client for training operations with optimized defaults."""
    ...
    async def forward_backward(
        ...
        """Compute gradients for a batch (async).
    ...
    async def train_batch(
        ...
        """Train on a single batch (async convenience method).
        ...
        """
        # Forward-backward
        fb_result = await self.forward_backward(
            batch_data=batch_data,
            accumulate=False,
            loss_fn=loss_fn,
            loss_kwargs=loss_kwargs,
        ...
        # Optimizer step
        opt_result = await self.optim_step(learning_rate=learning_rate)
    ...
    async def train_epoch(
        ...
        """Train for one epoch over a dataloader (async).
        
        Args:
            dataloader: Async iterable of batch data
            learning_rate: Optional learning rate override
            loss_fn: Loss function to use
            loss_kwargs: Additional arguments for loss function
