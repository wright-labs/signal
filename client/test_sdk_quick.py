...
def test_advanced_inference_api():
    ...
    with patch('frontier_signal.client.requests.Session') as mock_session_class:
        ...
        print(f"✓ Inference client created (step={inference.step}, batch_size={inference.batch_size})")
        ...
        # Test batch sample
        mock_response.json.return_value = {
            "outputs": [f"Response {i+1}" for i in range(10)]
        }
        ...
        output1 = inference.sample(["Same prompt"], max_tokens=50)
        
        # Second call (should use cache)
        output2 = inference.sample(["Same prompt"], max_tokens=50)
...
def test_progressive_disclosure():
    ...
    with patch('frontier_signal.client.requests.Session') as mock_session_class:
        ...
        inference = client.inference(run.run_id)
