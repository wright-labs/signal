...
async def main():
    ...
    async with AsyncSignalClient(
        ...
        print(
            f"✓ Request 1 completed ({(time.time() - start) * 1000:.1f}ms): loss={result1['loss']:.4f}"
        )

        result2 = await future2
        print(
            f"✓ Request 2 completed ({(time.time() - start) * 1000:.1f}ms): loss={result2['loss']:.4f}"
        )
        ...
        batches = [[{"text": f"Training example {i}"}] for i in range(5)]
