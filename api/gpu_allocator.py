...
GPU_MEMORY_CAPACITIES = {
    "L40S": 48, "A100-80GB": 80, "H100": 80, "T4": 16, "A10G": 24
}
SAFETY_MARGIN = 1.2
...
MODEL_REGISTRY: Dict[str, ModelInfo] = {
    # LLaMA family
    "meta-llama/Llama-3.2-1B": ModelInfo("meta-llama/Llama-3.2-1B", 1.23, "llama", 128000),
    "meta-llama/Llama-3.2-3B": ModelInfo("meta-llama/Llama-3.2-3B", 3.2, "llama", 128000),
    "meta-llama/Llama-3.1-8B": ModelInfo("meta-llama/Llama-3.1-8B", 8.0, "llama", 128000),
    "meta-llama/Llama-3.1-70B": ModelInfo("meta-llama/Llama-3.1-70B", 70.0, "llama", 128000),
...
def calculate_memory_requirements(
    ...
    activation_memory_gb = (
        config.batch_size * config.sequence_length * estimated_layers * bytes_per_param / (1024**3)
    )
    
    # Batch memory
    batch_memory_gb = config.batch_size * config.sequence_length * bytes_per_param / (1024**3)
    
    # Apply training type multiplier and safety margin
    base_memory = model_memory_gb + optimizer_memory_gb + activation_memory_gb + batch_memory_gb
...
def allocate_gpu_config(
    ...
    **kwargs
    ...
    logger.info(f"Auto-allocated {gpu_config} for {model_name} ({model_info.parameters_billions}B params)")
    return gpu_config
