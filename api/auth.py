...
class AuthManager:    
        ...
        try:
            if is_valid_format:
                ...
                result = self.supabase.table("api_keys").select(
                    "id, user_id, key_hash, is_active, expires_at"
                    ...
                    for key_data in result.data:
                        ...
                        if key_data.get("expires_at"):
                            ...
                            if datetime.now(expires_at.tzinfo) >= expires_at:
                                logger.info(f"Expired API key attempted: {key_data['id']}")
                                continue
                        ...
                        if self._verify_key_hash(api_key, key_data["key_hash"]):
                            ...
                            try:
                                user_check = self.supabase.table("profiles").select("id").eq("id", user_id).execute()
                            ...
                            try:
                                ...
                                logger.warning(f"Failed to update last_used for key {key_data['id']}: {e}")
    ...
    async def validate_jwt_token(self, token: str) -> Optional[str]:
        ...
        try:
            ...
            if not jwt_secret:
                logger.warning("FRONTIER_JWT_SECRET not configured, JWT validation disabled")
                await self._normalize_timing(start_time)
                return None
